# ============================================
# [Member 1] red_sign_segmentation.py
# ============================================
# Module: Red Sign Segmentation Using Color Information
# Owner: Member 1
#
# Purpose:
#   - Python replica of the C++ red_sign_segmentation.cpp code.
#   - Bypasses Windows Application Control policy blocking custom .exe files.
#   - Generates 6-panel grid output images for Report Chapter 4.
#
# Usage:
#   python red_sign_segmentation.py
# ============================================
...
def process_image(src, filename, out_path, clahe, show_steps=False):
    # Resize to 300x300 for consistent layout
    img = cv2.resize(src, (300, 300))
    black = np.zeros_like(img)
    
    # 1. Get binary mask
    red_mask = get_red_mask(img, clahe)
    ...
    for cnt in contours:
        ...
        if area > 500:  # Consistent area thresholding (adjustable to 300 if needed)
    ...
    # Initialize 6 panels
    p1 = img.copy()
    p2 = cv2.cvtColor(red_mask, cv2.COLOR_GRAY2BGR)    # Red Mask
    p3 = black.copy()                                 # All Contours
    p4 = black.copy()                                 # Largest Contour (Outline)
    ...
    if len(valid_contours) > 0:
        ...
        if max_area > 500:
            # Apply Convex Hull to fix fragmented/broken sign contours (from user code)
            ...
            # check 1: touches or is within 1 pixel of any image edge (more lenient for large/close-up signs)
            touches_edge = False
            lx, ly, lw, lh = cv2.boundingRect(hull)
            if lx <= 1 or ly <= 1 or (lx + lw) >= 299 or (ly + lh) >= 299:
                touches_edge = True
                
            # check 2: occupies more than 75% of total image area (allow large close-up signs)
            too_large = max_area > (90000 * 0.75)
    ...
    #    Top row: Original, Red Mask, All Contours
    ...
    if show_steps:
        win_name = "Red Sign Segmentation - " + filename
    ...
    if detected:
        print(f"  [{filename}] Red Sign Detected - {'SUCCESS' if success else 'FAILED'}")
    else:
        print(f"  [{filename}] No Red Sign Detected - FAILED")
        
    return detected, success

def main():
    print("=" * 40)
    print(" Member 1: Red Sign Segmentation (Python)")
    ...
    if not base_dir:
        print("ERROR: Could not locate directory 'Color Inputs/Red Signs'!")
        sys.exit(-1)
    ...
    for filename in sorted(os.listdir(base_dir)):
        ...
        det, succ = process_image(img, filename, out_path, clahe, show_steps)
        if det:
            total_detected += 1
        if succ:
            total_successful += 1
    ...
    print("\n" + "=" * 40)
    print(" RED SIGN SEGMENTATION SUMMARY")
    print("=" * 40)
    print(f" Total images processed: {total_images}")
    print(f" Total red signs detected: {total_detected}")
